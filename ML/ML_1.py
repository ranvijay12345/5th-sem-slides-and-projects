...
import numpy as np
import random
def gradient_descent(alpha, x, y, ep=0.0001, max_iter=10): # ep = rho
    converged = False
    iter = 0
    m = x.shape[0] # m = no of patterns/objects 
    weight = np.ones(x.shape[1]+1) # no of weights = no of features + 1
    random.seed(10) # same weight intilization
    for i in range(0,x.shape[1]+1):
      weight[i] = random.uniform(-0.3, 0.3) # randomly generates float no 
    total_error = 0
    temp_in = 0
    for i in range(0,m): # calculating mse
      for j in range(0,x.shape[1]):
        temp_in = temp_in + weight[j+1]*x[i][j]
      total_error = total_error + (weight[0] + temp_in - y[i])**2
    sum_n = np.ones(x.shape[1]+1)
    while not converged: # loop 
        error = 0
        temp_in = 0
        temp_out = 0
        for i in range(0,m): # hypothesis ( for weight_0)
          for j in range(0,x.shape[1]):
            temp_in = temp_in + weight[j+1]*x[i][j]
          temp_out = temp_out + (weight[0] + temp_in - y[j])*1
        sum_n[0] = temp_out

        for k in range(1,x.shape[1]): # hypothesis ( for rest weights)
          temp_in = 0 
          temp_out = 0
          for i in range(0,m):
            for j in range(0,x.shape[1]):
          ...
          sum_n[k+1] = temp_out
        for j in range(0,x.shape[1]): # weight updation
          weight[j] = weight[j] - (alpha/m) * sum_n[j]
        for i in range(0,m): 
          for j in range(0,x.shape[1]):
            temp_in = temp_in + weight[j+1]*x[i][j]
...
'''
# 1st hyper-parameter test (validation set)
